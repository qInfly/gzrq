...
/****************标志着当前小区管理机处理的集中器*********************/
        ...
        void setCoSid(const string& co_sid) {
                _co_sid = co_sid;
        }
...
private:
        ...
        void UpdateAllInfo(u8* ptr, const u32& origin_ptr_len);

        int checkMsgType(u8* data, size_t& data_len);

        void DataProcessing(u8* ptr);
